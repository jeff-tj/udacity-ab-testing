...
Therefore, our evaluation metrics could be:
* Gross conversion
* Retention
* Net conversion
...
One other scenario to consider could be an increase in net conversion combined with no change in retention - this might represent a scenario where the question actually serves to help students pre-commit a set amount of time to the course. Hence we should see an increase in the number of students who stay enrolled, but this would not effect the number who complete the checkout.
...
The main risk here is that the introduction of an additional question would hinderence to people signing up and therefore reduce the number of enrollments. However, I feel that the risk here is quite low - a person signing up would be required to enter main details (name, address, payment method, etc) and so one extra question would not pose much of a risk. Nor do I think that the question is likely to create learned effects that would impact the result of the experiment (if anything the behaviour change of discouraging time poor students from enrolling is one of the expected out comes of the experiment).

I would classify the experiment as low risk, therefore I feel it would be reasonable to divert 50% of regular traffic to the experiment.
...
* Gross conversion
* Net conversion
as the evaluation metrics when evaluating the experimental data. We retain the above invariant metrics. Having eliminated retention, we will require 679,300 pageviews. And we would expect the duration of the experiment to be 34 days.
...
Therefore I would recommend going ahead and implementing thew change, given that it does not significantly impact the proportion of students paying, but it does reduce significantly the number signing up for a free trial - which would help improve Udacity delivery by reducing resources spent on students who do not think they will have time to complete a course.
...
Therefore out unit of diversion should be user-id, since we want to track which of the students currently signed up receive a follow up email newsletter. 
